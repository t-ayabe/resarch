...
# 全画面表示でスクリーンを作成
dis = pygame.display.set_mode((0, 0), pygame.FULLSCREEN | pygame.DOUBLEBUF) # (0,0)を指定することで、現在のディスプレイの解像度を使用します
info = pygame.display.Info() # ウィンドウ位置とモードを設定した後に情報を取得します
...
inch = 17.3 # これは物理的な対角サイズを表すプレースホルダーの値かもしれません。動的にするなら再考が必要です。
dpi = (SCREEN_HEIGHT**2 + SCREEN_WIDTH**2)**(1/2) / inch # このDPI計算は、「inch」が画面の対角線のサイズ（インチ）であると仮定しています。
...
# --- 映像用の変数 ---
moving_point = None
FISH_IMAGE_PATH = r'C:\Users\tayav\Documents\resarch\company\medaka.png' # 魚の画像パス
...
def loop():
    ...
    while True:
        ...
        if current_game_state == GAME_STATE_START_MENU:
            ...
            dis.fill(colors.BLACK) # ゲームプレイ中の背景色
